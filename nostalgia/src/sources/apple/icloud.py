...
from nostalgia.src.common.meta.aspect.place import Place
    ...
    def ingest(self, delta_data, **kwargs) -> pd.DataFrame:
        return pd.concat(
            [
                ...
                for f in just.glob("~/nostalgia_data/input/apple/*/iCloudUsageData Set*.csv")
            ]
        )

    def load(self, data):
        data = data.iloc[
            : data.loc[data.Date == "Photos: Delete photo/video from iCloud Photo Library"].index.to_list()[0]
        ]
